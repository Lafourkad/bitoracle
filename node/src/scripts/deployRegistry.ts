/**
 * deployRegistry.ts — Redéploie uniquement OracleRegistry
 */
...
console.log('[deploy Registry] Wallet:', wallet.p2tr);
...
const bytecode  = readFileSync(join(ROOT, '../contracts/build/OracleRegistry.wasm'));
...
const fr = await provider.sendRawTransaction(fundingTx, false);
console.log('Funding:', fr.success ? '✅' : '❌', fr.result);
if (!fr.success) throw new Error('Funding failed: ' + fr.result);

await new Promise(r => setTimeout(r, 2000));

const dr = await provider.sendRawTransaction(deployTx, false);
console.log('Deploy:', dr.success ? '✅' : '❌', dr.result);
if (!dr.success) throw new Error('Deploy failed: ' + dr.result);

console.log('OracleRegistry:', result.contractAddress);

// Update deployment.json
const dep = JSON.parse(readFileSync(join(ROOT, 'keys/deployment.json'), 'utf8'));
dep.oracleRegistryAddress = result.contractAddress;
dep.oracleRegistryTxid    = dr.result;
